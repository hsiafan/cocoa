...
func (c *Class) GoImports() set.Set[string] {
	...
	if c.Parent != nil {
		imports.Add("github.com/hsiafan/cocoa/" + c.Parent.Type.Module.Package)
	}
...
func (c *Class) writeGoStruct(w *CodeWriter) {
	...
	for _, m := range c.InstanceTypeMethods {
		w.WriteLine("")
		im := m.NormalizeInstanceTypeMethod(c.Type)
		im.WriteGoCallCode(c.Type.Module, c.Type.GName, w)
		if im.Name == "new" {
	...
	for _, m := range c.Methods {
		w.WriteLine("")
		m.WriteGoCallCode(c.Type.Module, c.Type.GName, w)
	}
